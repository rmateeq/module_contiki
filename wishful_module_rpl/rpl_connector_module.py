...
class ContikiIPv6Connector(wishful_module.AgentModule):

    def __init__(self, **kwargs):
        super(ContikiIPv6Connector, self).__init__()
        self.log = logging.getLogger('ContikiIPv6')
    ...
    def start_ContikiIPv6_connector(self):
        for iface in self.supported_interfaces:
            node = self.node_factory.get_node(iface)
            connector = ProtocolConnector(1, "ContikiIPv6")
    ...
    def get_network_parameters(self, param_key_list):
        ...
        if node is not None:
            return node.read_parameters('ContikiIPv6', param_key_list)
        else:
    ...
    def set_network_parameters(self, param_key_values_dict):
        node = self.node_factory.get_node(self.interface)
        if node is not None:
            return node.write_parameters('ContikiIPv6', param_key_values_dict)
        else:
            fname = inspect.currentframe().f_code.co_name
            self.log.fatal("%s Interface %s does not exist!" % (self.interface, fname))
            raise exceptions.InvalidArgumentException(func_name=fname, err_msg="Interface does not exist")
    ...
    def get_network_measurements(self, measurement_key_list):
        node = self.node_factory.get_node(self.interface)
        if node is not None:
            return node.read_measurements('ContikiIPv6', measurement_key_list)
        else:
            fname = inspect.currentframe().f_code.co_name
            self.log.fatal("%s Interface %s does not exist!" % (self.interface, fname))
            raise exceptions.InvalidArgumentException(func_name=fname, err_msg="Interface does not exist")
    ...
    def get_network_measurements_periodic_worker(self, node, measurement_keys, collect_period, report_period, num_iterations, report_callback):
        ...
        for i in xrange(0, num_iterations):
            ...
            for i in xrange(0, num_collects_report):
                ...
                ret = node.read_measurements('ContikiIPv6', measurement_keys)
                for key in ret.keys():
    ...
    def define_network_event(self, event_key_list, event_callback, event_duration):
        ...
        if node is not None:
            try:
                return node.add_events_subscriber('ContikiIPv6', event_key_list, event_callback, event_duration)
            except Exception:
    ...
    def get_network_info(self):
        node = self.node_factory.get_node(self.interface)
        if node is not None:
            return NetworkInfo(self.node.ip_addr, node.params_name_dct['ContikiIPv6'].keys(), node.measurements_name_dct['ContikiIPv6'].keys(), node.events_name_dct['ContikiIPv6'].keys())
        else:
            fname = inspect.currentframe().f_code.co_name
            self.log.fatal("%s Interface %s does not exist!" % (self.interface, fname))
            raise exceptions.InvalidArgumentException(func_name=fname, err_msg="Interface does not exist")
