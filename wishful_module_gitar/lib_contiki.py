...
class ContikiNode(SensorNode):
	...
	def register_parameters(self,connector_module,param_defs):
		...
		for param_def in param_defs:
			if param_def['type_name'] == "STRUCT_T":
				...
				tlen = -1
				tformat = ""
			param = SensorParameter(param_def['unique_name'],int(param_def['unique_id']),param_def['type_name'],int(tlen),tformat)
	...
	def write_parameters(self,connector_module,param_key_values):
		...
		for key in param_key_values:
			if self.params_name_dct.has_key(connector_module) and self.params_name_dct[connector_module].has_key(key):
				...
				self.log.info("ContikiNode %s- write_parameters: parameter \"%s\" not found for connector %s", self.interface,key,connector_module)
	...
	def read_parameters(self,connector_module,param_keys):
		...
		for key in param_keys:
			if self.params_name_dct.has_key(connector_module) and self.params_name_dct[connector_module].has_key(key):
				...
				self.log.info("ContikiNode %s- read_parameters: parameter \"%s\" not found for connector %s", self.interface,key,connector_module)
		if message_hdr.num_args>0:
			message = bytearray(message_hdr.to_bin()) + message
			resp_message = self.__send_serial_cmd(4,message,message_hdr)
			if type(resp_message) == bytearray:			
	...
	def register_measurements(self,connector_module,measurement_defs):
		...
		for measurement_def in measurement_defs:
			if measurement_def['type_name'] == "STRUCT_T":
				...
				tlen = -1
				tformat = ""
			measurement = SensorMeasurement(measurement_def['unique_name'],int(measurement_def['unique_id']),measurement_def['type_name'],int(tlen),tformat)
	...
	def read_measurements(self,connector_module,measurement_keys):
		...
		for key in measurement_keys:
			if self.measurements_name_dct.has_key(connector_module) and self.measurements_name_dct[connector_module].has_key(key):
				...
				self.log.info("ContikiNode %s- read_measurements: measurement \"%s\" not found for connector %s", self.interface,key,connector_module)
		if message_hdr.num_args>0:
			message = bytearray(message_hdr.to_bin()) + message
			resp_message = self.__send_serial_cmd(4,message,message_hdr)
			if type(resp_message) == bytearray:			
	...
	def register_events(self,connector_module,event_defs):
		...
		for event_def in event_defs:
			if event_def['type_name'] == "STRUCT_T":
				...
				tlen = -1
				tformat = ""
			event = SensorEvent(event_def['unique_name'],int(event_def['unique_id']),event_def['type_name'],int(tlen),tformat)
	...
	def add_events_subscriber(self,connector_module,event_keys,event_callback):
		...
		for key in event_keys:
			if self.events_name_dct.has_key(connector_module) and self.events_name_dct[connector_module].has_key(key):
				...
				self.log.info("ContikiNode %s- add_events_subscriber: event \"%s\" not found for connector %s", self.interface,key,connector_module)
		if message_hdr.num_args>0:
			message = bytearray(message_hdr.to_bin()) + message
			resp_message = self.__send_serial_cmd(4,message,message_hdr)
			if type(resp_message) == bytearray:			
	...
	def __send_serial_cmd(self,max_attempts,message, message_hdr):
		...
		while num_attempts < max_attempts:
			...
			if self.__await_command_response() and self.__response_message != None:
